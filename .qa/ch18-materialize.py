...
EXPECTED_PACKAGE_SHA256 = "534f25c944a34e607bf190667416cc1fec3c1275a7524e7fedfa510d4b5a5505"
PACKAGED_BASE_COMMIT = "516292caf92f97d0e302d2f158c80436fa56e245"
ROOT = Path(".")
parts = sorted((ROOT / ".qa").glob("ch18-package-*.txt"))
if not parts:
    raise RuntimeError("Aucun fragment du paquet chapitre 18.")
...
if actual_package_sha != EXPECTED_PACKAGE_SHA256:
    raise RuntimeError(f"Paquet chapitre 18 corrompu: {actual_package_sha}")
...
if payload.get("schema") != 1 or not isinstance(payload.get("files"), dict):
    raise RuntimeError("Schéma du paquet chapitre 18 invalide.")
...
proof = ROOT / "Livre-III/QA/VALIDATION-FINALE-CHAPITRE-18.yaml"
...
print(
    f"Chapitre 18 matérialisé: {len(payload['files'])} fichiers vérifiés; "
