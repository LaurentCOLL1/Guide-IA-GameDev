...
func validate() -> PackedStringArray:
	...
	if species_id.strip_edges().is_empty():
		errors.append("species_id vide")
	if scene == null:
		errors.append("scene absente")
	if base_vitality < 1:
		errors.append("base_vitality inférieur à 1")
	if base_endurance < 1:
		errors.append("base_endurance inférieur à 1")
	if base_agility < 1:
		errors.append("base_agility inférieur à 1")
	if base_intellect < 1:
		errors.append("base_intellect inférieur à 1")
...
func register_definition(definition: CharacterDefinition) -> void:
	if definition == null:
...
- `queue_free()` planifie la destruction sûre du nœud ;
...
func clear_without_freeing() -> void:
	_runtime_by_id.clear()
	_body_by_id.clear()
```
...
func decode(
	...
	var character_id := String(value.get("character_id", ""))
	var definition_id := StringName(value.get("definition_id", ""))
	if not CharacterId.is_valid(character_id):
	...
	state.custom_name = String(value.get("custom_name", ""))
	state.current_health = int(value.get("current_health", -1))
	state.current_stamina = int(value.get("current_stamina", -1))
	state.vitality_bonus = int(bonuses.get("vitality", 0))
	state.endurance_bonus = int(bonuses.get("endurance", 0))
	state.agility_bonus = int(bonuses.get("agility", 0))
	state.intellect_bonus = int(bonuses.get("intellect", 0))
	state.world_position = Vector3(
		float(raw_position[0]),
		float(raw_position[1]),
		float(raw_position[2]),
	)
	state.world_yaw = float(world.get("yaw", 0.0))
	state.is_alive = bool(value.get("is_alive", false))
...
func register_definitions(
	...
	return catalog.all_ids().size() == definitions.size()
...
func create_character(
	...
	if not custom_name.strip_edges().is_empty():
		state.custom_name = custom_name.strip_edges()
